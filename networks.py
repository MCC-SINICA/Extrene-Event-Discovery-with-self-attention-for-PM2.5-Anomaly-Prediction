...
class Seq2Seq(nn.Module):
    ...
    def forward(self, past_window, xs, teacher_force_ratio=0.6):
        ...
        outputs     = torch.zeros(batch_size, trg_size, output_size).cuda()
        # Encode
...
class Fudan(nn.Module):
    ...
    def forward(self, x, past_window, past_ext):
        ...
        windows = torch.zeros(past_window.shape[0], past_window.shape[1], x_hidden.shape[-1]).cuda()
...
class Merged_Model(nn.Module):
    def __init__(self, opt, nor_model, ext_model):
        ...
        self.hid_out   = nn.Linear(opt.hid_dim * opt.source_size, opt.hid_dim * opt.target_size) if opt.model.lower() == "xfmr" else \
